...
parses through the generated argument table. The ten fixtures the generator **refuses** a decoder
for are named with their reason (tuples and multi-field union cases — the two combinator gaps
Phase 800 owns — and `DateOnly` / `TimeOnly`, which have no cross-host combinator). The program
...
| Path | Result |
|---|---|
| Generated decoders (binary response) | 58 of 58 expressible fixtures decode to their declaration; the 10 refusals are the algebra's recorded reach |
