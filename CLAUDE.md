...
- **Community** bindings: the standalone `Feliz.AgGrid` + `Feliz.AgCharts` packages — [`src/Feliz.AgGrid/AgGrid.fs`](src/Feliz.AgGrid/AgGrid.fs) + [`AgChart.fs`](src/Feliz.AgCharts/AgChart.fs). `ToolUp.Platform.Client` re-exports both under their pre-Phase-344 module names (`ToolUp.Platform.AgGrid` / `.AgChart`) so existing call sites compile unchanged (GP 11); new code opens `Feliz.AgGrid` / `Feliz.AgCharts`. Cookbook: [`src/ToolUp.Platform.Client/Client/UI/COOKBOOK.md`](src/ToolUp.Platform.Client/Client/UI/COOKBOOK.md) — the canonical authoring reference (constraints → shortest-possible → recipes → anti-patterns), single-Read for an AI agent.
