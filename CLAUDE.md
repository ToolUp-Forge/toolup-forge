...
**Release candidates, and the soak gate a major is promoted through (Phase 263).** A tag `vX.Y.Z-rc.N` publishes the lockstep packages to nuget.org as the SemVer-2 prerelease `X.Y.Z-rc.N` — the tree keeps declaring the bare `X.Y.Z`; the label lives only on the tag, and a consumer that does not opt into prereleases never resolves one. `publish-nuget.yml` runs `VerifyReleaseChannel` before minting a credential: it refuses any tag shape other than `vX.Y.Z` / `vX.Y.Z-rc.N` and any tag that does not name the tree's `<Version>` (before it, every tag published `<Version>` whatever it said). A new major (`X.0.0`, X ≥ 1) — and any version that has a candidate — is promoted only from a candidate that nuget.org has served for 14 days with no public-surface movement since, from a descendant commit, with the Phase 257 scorecard all-green (`dotnet run --project Build.fsproj -- VerifyRcPromotion` before tagging). Rules in `ReleaseChannel.fs`; the operator flow in [`docs/platform/release-process.md`](docs/platform/release-process.md).
