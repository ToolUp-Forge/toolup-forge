...
2. **`DataTypeDisplay.RenderSummary`** — every data-producing module boxes its summary record in its server-side `DataType.Process` and unboxes in the client-side `RenderSummary` callback. Symmetric same-module-known-type cast on both ends. **Retiring (Phase 817): `ProcessedFileEntry.Summary` carries the summary as a `ProcessedData` envelope and `DataTypeDisplay.typed` decodes it, so nothing is boxed; `Info` and `RenderSummary` are `[<Obsolete>]` and go in 1.0, at which point this boundary is closed and `FileManagementApi` — the last platform API record on the reflection decode path — joins the proved one.** Until then the legacy path renders exactly as before.
