...
let mintMac (key: byte[]) (keyId: string) (moduleId: string) : JsonObject =
    ...
    let tag = base64Url (hmac.ComputeHash(canonicalBytes moduleId))
...
/// with the supplied P-256 private key. The JWS protected header is
/// `{"alg":"ES256","kid":<keyId>,"typ":"JOSE"}`; the detached JWS is
/// `base64url(header) + ".." + base64url(r‖s)`.
let mintJws (ec: ECDsa) (keyId: string) (moduleId: string) : JsonObject =
    ...
    let signingInput =
        Encoding.UTF8.GetBytes(encodedHeader + "." + base64Url (canonicalBytes moduleId))
...
let rec private parse (opts: Options) (args: string list) : Result<Options, string> =
    ...
    | ("--manifest" | "--module" | "--key-id" | "--mac-key" | "--mac-key-file" | "--ec-key-file") :: [] ->
...
/// Resolve the key source into a minting function `moduleId -> JsonObject`.
let private resolveMinter (keyId: string) (source: KeySource) : Result<(string -> JsonObject) * IDisposable, string> =
    ...
    | EcKeyFile f ->
        if not (File.Exists f) then
            ...
            try
                ...
                Ok((fun m -> mintJws ec keyId m), (ec :> IDisposable))
            with ex ->
