...
let tests =
    testList "ToolUp.AI.Wire (Fable host)" [
        ...
        testList "Gemini wire mapping (Fable host)" [
            ...
            testCase "parseResponse — text + usage" (fun () ->
                ...
                | Ok r ->
                    ...
                    match r.Usage with
                    | Some u ->
                        Expect.equal u.PromptTokens GeminiWireFixtures.textResponseExpectedPromptTokens "prompt tokens"
                        Expect.equal u.CachedPromptTokens GeminiWireFixtures.textResponseExpectedCachedTokens "cached"
                        Expect.equal u.OutputTokens GeminiWireFixtures.textResponseExpectedOutputTokens "output tokens"
        ...
        // Phase 254 — the Claude portable mapper Fable-compiles; assert
        // buildRequestBody emits bytes identical to the .NET host over the
        // shared golden fixtures (the byte-parity gate), parseResponse reifies
        // the identical shape, and the pure ClaudeStreaming machine assembles
        // a canned SSE chunk sequence (incl. the post-stream {} default-fill)
        // identically on this host.
        testList "Claude wire mapping (Fable host)" [
            testList "buildRequestBody (byte-stable golden)" [
                for name, build, golden in ClaudeWireFixtures.requestFixtures do
                    testCase (sprintf "request %s" name) (fun () -> Expect.equal (build ()) golden "request body bytes")
            ]

            testCase "parseResponse — text + tool_use + usage" (fun () ->
            ...
            testCase "streaming — text + tool-call assembly + split usage" (fun () ->
            ...
            testCase "streaming — zero-input tool call defaults Arguments to {}" (fun () ->
                let response, emitted =
