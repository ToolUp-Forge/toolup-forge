...
// (`CsrfClient.setApiOrigin`). The shipped topology serves the SPA and
...
// on in that shape and the composition root has NOT called
// `CsrfClient.setApiOrigin`, every state-changing `/api/*` call 403s
...
// split-origin deployments need `setApiOrigin`. Operators who know
...
/// reminding the operator that a cross-origin SPA must call
/// `CsrfClient.setApiOrigin` or the client request-guard will withhold
