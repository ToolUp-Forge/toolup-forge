...
/// serializer for `ToolUp.Remoting.Server.Remoting.createApi()` is now
/// `ToolUp.Remoting.Json.SystemTextJson.FableConverters.create()` — a
...
/// `Newtonsoft.Json` package reference is dropped from `ToolUp.Remoting.Json`.
/// See MIGRATION.md for the migration path.
[<System.Obsolete "Use ToolUp.Remoting.Json.SystemTextJson.FableConverters.create() — produces byte-equal wire output via System.Text.Json with no Newtonsoft.Json transitive dep. See MIGRATION.md.">]
