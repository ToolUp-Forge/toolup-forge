...
module ThreadSafeCell =
    ...
    let create () =
        ...
        <| fun inbox ->
            let rec loop state = async {
                ...
                match msg with
                | Set value -> return! loop (Some value)
                | Get channel ->
                    channel.Reply state
                    return! loop state
            }
    ...
    let set (value: 't) (cell: MailboxProcessor<CellOperations<'t>>) = cell.Post(Set value)

    let get (cell: MailboxProcessor<CellOperations<'t>>) = cell.PostAndAsyncReply Get
    ...
    let computeOnce (f: unit -> 't) : (unit -> Async<'t>) =
        ...
        fun () -> async {
            ...
            | None ->
                set computed atom
                return computed
            | Some previouslyComputedValue -> return previouslyComputedValue
        }