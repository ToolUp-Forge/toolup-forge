...
module Diagnostics =
    ...
    let runPhase logger text =
        logger
        |> Option.iter (fun logf ->
            StringBuilder()
            |> writeLn (sprintf "ToolUp.Remoting: invoking function %s" text)
            |> toLogger logf)
    ...
    let outputPhase logger value =
        ...
        |> Option.iter (fun logf ->
            StringBuilder()
            |> writeLn "ToolUp.Remoting: Returning serialized result back to client"
            |> writeLn value
            |> toLogger logf)