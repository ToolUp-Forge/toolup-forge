...
// ─── Authorize-request parameter set ─────────────────────────────────
//
// The query the sign-in redirect carries. Pure and separated from
// `beginSignInWithExtras` for two reasons: .NET-side Expecto can then
// assert the EXACT parameter set a given flow produces (which is how
// the secondary-flow affordance is parity-checked against the Entra
// companion's shell it replaces), and the reserved-key list the
// coherence validator's rule 16 enforces has something to be pinned
// against rather than being a second copy of this knowledge.

/// The per-attempt values `beginSignInWithExtras` has generated by the
/// time it builds the query: the config's stable identifiers plus the
/// freshly-minted `state`, `nonce` and PKCE challenge.
...
/// The complete authorize-request parameter set in wire order: the
/// standard OAuth / PKCE parameters first, then the flow's extras.
///
/// `extras` is what distinguishes one flow from another — the primary
/// "Sign in" button passes `[]`, a secondary flow passes its
/// `ExtraAuthorizeParams` — so both buttons produce the identical
/// standard set and differ in exactly the parameters the deployment
/// declared. Extras are appended, never merged: a key that collides
/// with a standard one appears twice, which is why
/// `SecondaryFlow.reservedAuthorizeParams` exists and why the
/// coherence validator refuses such a config rather than this function
/// silently dropping one side of the collision.
let authorizeParams (req: AuthorizeRequest) (extras: (string * string) list) : (string * string) list =
    [
        "response_type", "code"
        "client_id", req.ClientId
        "redirect_uri", req.RedirectUri
        "scope", String.concat " " req.Scopes
        "state", req.State
        "nonce", req.Nonce
