...
- **Detection is compose-time, from the deployment's tool list.** `composeRAG` reads the aggregated `ServerApp.AITools` and derives a `RAGPromptBuilder.ToolFraming` via `ToolFraming.fromTools`. A tool counts as *live-interface* when it declares `IsLiveInterface = true` **or** its `Location = ClientResident`. Server-resident analytical tools — including the `_platform.ai.*` cross-module read family — are **not** live-interface tools: they read persisted data, so the KB-first framing still applies to them. A deployment with no such tools yields `HasLiveUiTools = false` and sees the pre-Phase-14r framing byte-for-byte (no regression).

  **Phase 538 — the trigger is typed, not stringly-typed.** Until Phase 538 the derivation carried a third arm: a tool whose *name* started with `_platform.ui.` was treated as live-interface. That coupled forge's own framing to a naming convention forge never emits — the concrete `_platform.ui.*` tools live in an external host-adapter layer — so the SDK was recognising a string it does not produce. Two failure modes followed from it, and both are now closed: a live-interface tool named anything else (a third-party adapter's, or a forge-native one) silently missed the framing, and a *server-resident* tool that merely happened to carry the prefix wrongly tripped it. `AIToolDefinition.IsLiveInterface` replaces the name match with an explicit, provider-agnostic declaration; **no tool name is inspected by the framing derivation.** The `ClientResident` implication is retained unchanged, so a client-resident tool need not set the flag. The field defaults to `false`, so every pre-538 tool yields byte-identical framing (GP 11).
