...
/// `OidcClient` and `PasskeyClient` (Phase 751; the Entra External ID
/// client shell was a third until Phase 749 removed that companion, and
/// its deployments render `OidcClient`'s screens through the preset
/// path, so they are covered by this same section).
///
/// ONE shared section rather than one per package: they render the same
/// vocabulary, and near-identical sections would ask a translator for
/// "Sign in" once per package. `Passkey` carries the prompts
