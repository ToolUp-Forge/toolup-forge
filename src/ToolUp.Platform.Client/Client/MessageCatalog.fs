...
module MessageCatalog =
    ...
    let english: MessageCatalog = {
        ...
        ExternalContact = {
            Heading = "Contacts"
            Subheading =
                "People this team reaches who have no account here "
                + "—"
                + " clients, group members, family. Each is reachable only on the channels they have consented to."
            Loading = "Loading" + "…"
            Dismiss = "dismiss"
            ...
            ContactFormHelp =
                "An address on file is not permission to use it. Filing a contact records how to reach them; consent is recorded separately, per channel."
            ...
            DetailEmail = fun value -> $"Email: {value}"
            DetailPhone = fun value -> $"Phone: {value}"
            DetailTags = fun value -> $"Tags: {value}"
            EditContact = "Edit"
            ...
            ConsentHelp =
                "What this person has agreed to, and how that agreement was obtained. A channel not listed here cannot be used to reach them."
            NoConsent = "No consent"
            NoConsentBody = "No consent has been recorded, so this contact cannot be reached on any channel."
            ConsentEvidence = fun source -> $"Obtained: {source}"
            RecordConsentHelp =
                "Recording a consent asserts, on the record, that this person agreed to be contacted on this channel. Say how it was obtained "
                + "—"
                + " a form submission id, a signed form, or that an admin entered it by hand."
        ...
        CompositionInspector = {
            ...
            Subheading =
                "What this deployment composes, what rules it passed, what it may disclose and to whom, and where its numbers come from — read from the deployment's own declarations. Read-only, Owner / Admin only."
            ...
            Exporting = "Exporting…"
            Refresh = "Refresh"
            SectionCount = fun name count -> $"{name} ({count})"
            Modules = "Modules"
            ...
            CapturedAt = fun stamp -> $"Captured at startup: {stamp}"
            EnvelopeDigest = "Envelope digest (SHA-256)"
            NoDeclarations =
                "This composition declares no grounding metrics, subjects, purposes or disclosure policies. That is a complete answer, not a missing one — a deployment that registers none declares none."
            ...
            ProvenanceWalkNote =
                "The chain walk itself is served by this deployment's provenance query contract, which applies the export disclosure door and the declared depth and node caps. This panel reports which parts of that substrate are composed; it deliberately does not open a second walk over the same graph."
            NoneDeclared = fun name -> $"This deployment declares no {name}."
        }
        MappingDataManager = {
        ...
        Observability = {
            ...
            Loading = "Loading…"
            NoSources =
                "No self-hosted observability source is enabled in this deployment — enable the log store, the metrics history or an alert rule on the server."
            ErrorForbidden = "This view is limited to platform administrators."
            ErrorNotComposed =
                "This source is not available in this deployment — its server-side substrate is not composed."
            ErrorStatus = fun code -> $"The request failed (HTTP {code})."
            ErrorUnreadable = fun reason -> $"The response could not be read: {reason}"
            ErrorUnreachable = fun reason -> $"The request could not be sent: {reason}"
            ...
            LogsFootnote =
                "Newest first. Text search matches whole words, case-insensitively. Select a line to see its full record."
            DetailHeading = "Selected line"
            ShowCorrelated = "Show every line of this request"
            NoPoints = "No points in this window."
            SeriesCount = fun count -> $"{count} series combined"
            PartialRead = fun reason -> $"Some series could not be read: {reason}"
            MetricsFootnote =
                "From the metrics history. Counters are per-flush increments; percentile lines are the slowest tag set's, an upper bound rather than a fleet-wide percentile."
            ColumnRule = "Rule"
            ColumnSignal = "Signal"
            ...
            Minutes = fun minutes -> $"for {minutes} min"
            ...
            AlertsFootnote =
                "As this process's alert engine last evaluated each rule. A process that does not run the engine reports every rule as not evaluated."
        }
    }
