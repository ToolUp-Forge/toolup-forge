...
type Cell<'T> internal () =
    ...
    member __.Value: 'T =
        if isValueCreated then
            ...
            sprintf "Value for '%O' has not been initialized." typeof<'T> |> invalidOp

    member internal __.Value
        with set t =
            if isValueCreated then
                sprintf "Value for '%O' has already been initialized" typeof<'T> |> invalidOp
    ...
    interface ICell with
        ...
        member __.Value = box __.Value

        member __.Value
            with set t = __.Value <- unbox t



[<NoEquality; NoComparison>]
type private GenPayload = {
    Cell: ICell
    DelayedValue: obj
    Dependencies: HashSet<Type>
}

[<NoEquality; NoComparison>]
type private CachePayload = {
    Type: Type
    Value: obj
    Dependencies: HashSet<Type>
}
...
type TypeGenerationContext internal (parentCache: TypeCache option) =
    ...
    let acquire () =
        if isDisposed then
            raise <| ObjectDisposedException("TypeGenerationContext")

        if Interlocked.CompareExchange(&isAcquired, 1, 0) = 0 then
            { new IDisposable with
                member __.Dispose() = isAcquired <- 0
            }
        else
            invalidOp "Multi-threaded usage of TypeGenerationContext not supported."
    ...
    member __.Commit<'T> (_: GenerationToken<'T>) (value: 'T) : 'T =
        ...
        if stack.Count = 0 || stack.Pop() <> typeof<'T> then
            invalidOp "TypeGenerationContext: unexpected commit operation"
    ...
    member internal __.GetGeneratedValues() =
        ...
        if stack.Count > 0 then
            ...
            dict
            |> Seq.map (function
                | KeyValue(_, v) -> v.Cell.Type, v.Cell.Value, Seq.toArray v.Dependencies)
            |> Seq.toArray
...
and TypeCache private (cache: ConcurrentDictionary<Type, CachePayload>) =
    ...
    let clone (cache: ConcurrentDictionary<Type, CachePayload>) =
        ...
        |> Seq.map (fun kv ->
            ...
            let payload2 = {
                payload with
                    Dependencies = HashSet(payload.Dependencies)
            }
    ...
    member __.TryFind(t: Type) : obj option =
        let mutable p = Unchecked.defaultof<_>
        if cache.TryGetValue(t, &p) then Some p.Value else None
...
type private ShallowObjectCopier<'T> private () =
    static let copier: Lazy<Action<'T, 'T>> =
        lazy
            (let t = typeof<'T>

             if not t.IsClass then
                 invalidArg t.FullName "unsupported type"
