...
let deleteDocument (deps: KnowledgeApiDeps) (docId: string) : Async<Result<unit, string>> = async {
    ...
    | Some doc ->
        ...
        // Drop the document's chunks from the vector store so RAG
        // retrieval stops surfacing them. Chunk ids follow
        // `{docId}:chunk:{i}` (matches the upload + AddNote paths).
        // Falls through if no vector store is wired (tests).
