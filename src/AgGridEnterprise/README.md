...
Licensing-isolated from the core SDK so deployments running AG Grid Community don't accidentally ship Enterprise code. The Community-only path is the default; this companion is opt-in via `<Import Project="...\AgGridEnterprise.Client.props" />` + a runtime `AgGridEnterprise.register "<license-key>"` call in the client composition root.
