...
/// `IJobScheduler` stub for the fusion-on test. `JsonRpcPeerHost.contract`
...
/// In-memory `IPeerJobResultStore` for the fusion-on test. Captured by
/// the `PeerJobHandler` at construction time; never read by the pack.
type private InMemoryResultStore() =
    let store = ConcurrentDictionary<string * PeerJobId, PeerJobStatus<string>>()

    interface IPeerJobResultStore with
        member _.SaveResult(scopeId: string, jobId: PeerJobId, status: PeerJobStatus<string>) = async {
            store[(scopeId, jobId)] <- status
        }
