...
let tests (name: string) (factory: unit -> IBackgroundExportStore) =
    ...
    testList $"{name} — IBackgroundExportStore contract" [
        ...
        <| async {
            ...
            Expect.equal status Preparing "fresh ticket is Preparing"
        }
        ...
        <| async {
            ...
            let! status = store.GetStatus ticket
            Expect.equal status (Ready(int64 envelope.Length)) "Ready carries the size"
        ...
        <| async {
            ...
            let! status = store.GetStatus ticket
            Expect.equal status (Failed "exporter blew up") "Failed carries the reason"
            let! dl = store.Download ticket
        ...
        <| async {
            ...
            let! status = store.GetStatus ticket
            Expect.equal status Cancelled "Cancelled status"
            let! dl = store.Download ticket
        ...
        <| async {
            ...
            Expect.equal status Unknown "unknown ticket reports Unknown"
        ...
        <| async {
            ...
            Expect.equal s1 (Ready 3L) "t1 completed"
            Expect.equal s2 Preparing "t2 untouched by t1's completion"
        }
    ]