...
//
// `CleanRoomBroker.fs` enforces a floor on ONE answer: this cohort is at
// or above k, these cells are suppressed, this shape is permitted. It is
// a pure function of the answer in front of it and holds nothing between
// calls, which is exactly right for what it does and is also its limit —
// a counterparty that asks a hundred individually-compliant questions
// gets a hundred compliant answers and the broker never notices. Cohort
// floors do not compose: differencing two in-floor cohorts that overlap
// in all but one record recovers that record, and no per-query check can
// see it, because each query passed.
//
// This file adds the CUMULATIVE half — the epsilon (ε) budget a
// regulated clean-room buyer audits. It is stateful and durable where
// the broker is pure and stateless, and those are different jobs, which
// is why they are different seams rather than one interface doing both.
//
// ── What this DOES guarantee, stated narrowly ──
//
//   * A declared ε ceiling per (template, counterparty, epoch) is
//     enforced. Once spend reaches the ceiling every further answer under
//     that template is WITHHELD — a refusal, not a warning, on the same
//     structural path Phase 311 put the floor on, so a handler has no say
//     in it.
...
//     failure `BlobPeerReplayGuard` refuses a non-conditional backend to
//     avoid, and this file refuses one for the same reason.
...
// `ICleanRoomBroker` adds no noise — it suppresses small cells and
// refuses small cohorts, and both are deterministic. Charging ε for a
// deterministic answer and summing the charges bounds nothing formally;
// what it bounds is how many questions a counterparty may ask under a
// declared schedule, and it makes exhaustion enforced and auditable
// rather than discovered afterwards. Anyone who needs the formal
// guarantee must substitute an `ICleanRoomBroker` that actually
// randomises its answers, at which point these ε values become the
// mechanism's real privacy loss and this ledger becomes its accountant.
// Until then, calling the number "ε" is a naming convention borrowed
// from the accounting it resembles, and describing it to a regulator as
...
//     A receiver validates the peer it is talking to; the rest of a
...
// `PeerServerApp.withPrivacyBudget`. `NoPrivacyBudgetLedger` exists for
// the composition that wants the seam present and the accounting off;
// a composition that declares no meter allocates nothing and takes no
// branch on the dispatch path, exactly as Phase 480's absent approval
// registry does.
...
type BudgetScope = {
    /// `CleanRoomTemplate.TemplateId` — the query surface the budget
    /// protects.
    TemplateId: string
    /// `PeerId` of the *validated* calling peer. Never the self-asserted
    /// wire body, and never a cascade origin (see the file header).
...
type PrivacyBudgetPolicy = {
    ...
    /// transport deadline of 100 s (`PeerTransportPolicy.defaults`), so
    /// a reclaim cannot race a live call.
...
/// `ICleanRoomBroker`: a deployment whose ε accounting already lives in
/// a warehouse ships its own and keeps one book.
///
...
type IPrivacyBudgetLedger =
    ...
    /// `IPeerReplayGuard.IsDistributed` takes.
...
/// loudly and at compose time rather than at the first peer call, the
/// same posture `BlobPeerReplayGuard` takes for the same reason.
type BlobPrivacyBudgetLedger(blobs: IBlobStorage, now: unit -> DateTimeOffset) =
    ...
    let parse (bytes: byte[]) =
        try
            let parsed = JsonRpc.deserialize<LedgerState> (Encoding.UTF8.GetString bytes)
    ...
    let render (state: LedgerState) =
        Encoding.UTF8.GetBytes(JsonRpc.serialize state)
