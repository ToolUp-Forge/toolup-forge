...
**Dispatch side** (`scheduleDispatch`): schedules a `Manual`-trigger `JobRegistration` under scope `PeerJob.Scope = "_platform"`, handler name `PeerJob.handlerName contractId methodName` = `_platform.peer.{contractId}.{methodName}`, `CreatedBy = PeerJob.SourceModule = "_platform.peer"`, then `TriggerOnce` and returns the assigned `JobId` (serialised) for the caller to poll. Job payload is the call's argument JSON.

**Execution side** (`PeerJobHandler : IJobHandler`): the scheduler dispatches it; it unmarshals the call arguments from the job payload, applies the contract implementation, resolves the returned `PeerJobHandle<'T>` to a terminal state, and persists the serialised result. The handler always finishes the *job* as `Success` (the job of *capturing the terminal status* succeeded); a peer-side failure is recorded as a `Failed` status in the result store — **not** a job retry, because re-running a deterministic peer computation would double-execute it.

**Result store** (`IPeerJobResultStore` / `BlobPeerJobResultStore`): the job substrate's own `JobResult` carries no payload, so the typed (serialised) result is parked here keyed by `JobId`. Default layout: one JSON document per job under the reserved `_platform` container at `peers/jobs/{scopeId}/{jobId}.json`, mirroring `BlobPeerRegistry`. Absence of a stored status *is* the `Pending` signal — the poll route reports `None` as `Pending`.
...
- **Long-running result GC** — `BlobPeerJobResultStore` writes one document per job and never deletes; there is no TTL / sweep. A finished peer job's result blob persists until an external lifecycle removes it. (Mirrors the append-only `IResultStore` hygiene question tracked elsewhere.)
