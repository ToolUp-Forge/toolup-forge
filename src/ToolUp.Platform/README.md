...
| `TrustForwardedHeaders` | `false` | Adds `app.UseForwardedHeaders(...)` honouring `X-Forwarded-Proto` / `X-Forwarded-For` from any peer. Required behind a TLS-terminating reverse proxy so secure cookies, OIDC `RedirectUri`, and `Url.IsAbsoluteUri` see the originating scheme. | `TOOLUP_TRUST_FORWARDED_HEADERS` |
