...
A modular application framework for building analytical platforms in F# on ASP.NET Core + Giraffe (server), Fable + Feliz with the in-tree ToolUp.Elmish runtime (client), the in-tree ToolUp.Remoting transport (type-safe wire), and any-cloud blob / event storage.
...
- **No new side effects.** Refactors and new features must not introduce mutable global state or imperative side effects in module code. Where legacy side effects exist, prefer eliminating them over working around them.
  ...
  **Documented module-level mutable exceptions.** A small number of SDK-internal mutables exist for initialisation-time or cache state that is set once and read many times. These are the *only* sanctioned module-level mutables; new ones require an entry in this list and a justification.
  ...
  | `AgGrid.gridModulesRegistered` | `Feliz.AgGrid/AgGrid.fs:65` | Set-once flag preventing the Community-edition fallback when the AgGridEnterprise companion has registered Enterprise modules first | Initialisation-time guard. `AgGridEnterprise.register` flips it before `Client.run`; subsequent reads from `ensureGridModulesRegistered ()` short-circuit. The alternative (registering Community modules unconditionally then having Enterprise re-register) breaks AG Grid's module-registry invariants. |
  | `AgChart.chartsModulesRegistered` | `Feliz.AgCharts/AgChart.fs:19` | Same shape as `AgGrid.gridModulesRegistered` for AG Charts Community/Enterprise selection | Initialisation-time guard. Same precedent. |
  ...
  | `AgChart.ChartPalette.fills`/`strokes`/`accentColor`/`markerFill`/`markerStroke`/`fontFamily` | `Feliz.AgCharts/AgChart.fs:32-37` | Theme palette read at chart-construction time. `ClientConfig` chart-theme overrides flip these at boot; module views read them when building chart options | Initialisation-time theme tokens. Apps configure the palette once during `Client.run`'s setup phase; chart code reads it per-render. Threading a theme record through every `AgChart.chart` call site would balloon the API surface. |
...
| `TrustForwardedHeaders` | `true` (Phase 16d) | Adds `app.UseForwardedHeaders(...)` honouring `X-Forwarded-Proto` / `X-Forwarded-For`. Default-on since Phase 16d — containerised / serverless deploys are almost always behind a TLS-terminating ingress and need this on. Set `TOOLUP_TRUST_FORWARDED_HEADERS=0` (or `=false` / `=no` / `=off`) on a direct-bind dev shell with no proxy hop. Unrecognised values crash startup. **Phase 325:** on an auth-requiring surface, pair with `TrustedProxyCidrs` (below) or the escape hatch or preflight refuses to start. | `TOOLUP_TRUST_FORWARDED_HEADERS` |
| `TrustedProxyCidrs` | `[]` (Phase 325) | When non-empty, scopes forwarded-header trust to in-range peers via `KnownIPNetworks` (comma-separated CIDRs, e.g. `10.0.0.0/8`; IPv6 supported; malformed entry fails loud). Empty + `TrustForwardedHeaders = true` + no escape hatch is a preflight **Error in auth-requiring modes** (Anonymous keeps a Warning); the escape hatch is `TOOLUP_ACCEPT_FORWARDED_HEADERS_FROM_ANY_PROXY=1`. | `TOOLUP_TRUSTED_PROXY_CIDRS` |
...
- **Serialization.** `NotificationHandler` uses the `ToolUp.Remoting.Json` converter set, matching the rule for any manual server→Fable JSON. Named events (`event: SystemMessage`, etc.) map `NotificationKind` onto the SSE frame so the client can `addEventListener` per kind.
...
- **Vendor companions:**
  ...
  - `src/NotificationChannels/WhatsApp/Twilio/` — Phase 6f.B. The same Twilio Messages endpoint, account and `TWILIO_AUTH_TOKEN` as the SMS sink, `whatsapp:`-prefixed addresses; template sends map the registry's template name onto a Twilio content SID (`ContentSid` + `ContentVariables`), free-form sends carry `Body`. Recipients resolve through the consent-gated `INotificationAddressBook.ResolveWhatsApp`.
  - `src/NotificationChannels/WhatsApp/MetaCloud/` — Phase 6f.C. The Meta WhatsApp Business Cloud API (`/{phone-number-id}/messages` on the Graph API) over BCL `HttpClient`, no vendor SDK; System User token from `ISecretStore.GetSecret("_platform", "meta_whatsapp_access_token")`, read per send so rotation needs no restart. Template sends build Meta's `components` array from the `IWhatsAppTemplateRegistry` arity; free-form sends are `type: "text"`. Ships an inbound webhook handler (`whatsapp-meta`, on the `ToolUp.Webhooks` substrate, `X-Hub-Signature-256` verified) that stamps `ExternalContact.LastInboundUtc` — opening the 24-hour window — and audits `failed` delivery statuses. Recipients resolve through the consent-gated `INotificationAddressBook.ResolveWhatsApp`.
...
SDK server files that persist F# records and DUs losslessly (the config, data-object, result, lineage, audit, feature-flag, module-query, notification, and webhook stores) all serialise through the System.Text.Json converter set from `ToolUp.Remoting.Json.SystemTextJson.FableConverters.create ()`, which ships inside `ToolUp.Platform.Server`. A consuming server project's `PackageReference Include="ToolUp.Platform.Server"` is therefore sufficient — `System.Text.Json` itself ships in the BCL, and no separate JSON package reference is required. (`Newtonsoft.Json` is not a forge dependency; the legacy Newtonsoft-based `FableJsonConverter` was retired in the STJ migration.)

- **Why not bare `System.Text.Json`?** The default STJ contract does not round-trip F# DUs in the client-parseable shape — nullary cases and DU-with-payload shapes like `VersioningPolicy` (`Unversioned` / `Versioned` / `StrictlyVersioned`) and `LinkType` serialise to the `{"Case":"X","Fields":[...]}` contract shape `Fable.SimpleJson` cannot read. `FableConverters.create ()` registers the full F# converter set (option / DU / tuple / record / list / Map / Set / …) and produces exactly the wire format `Fable.SimpleJson` parses on the client.
- **Canonical helper shape**: `Server/Config/ConfigStore.fs` — construct the options once at module level (`let private options = FableConverters.create ()`) and reuse them for every `Serialize` / `Deserialize` call. New SDK files that persist F# data should copy that shape rather than re-deriving the converter wiring.
- **Symmetric on the client side.** Fable client projects that manually parse JSON emitted by these stores (or by SSE frames) reference `Fable.SimpleJson` — it reads the `FableConverters` wire format losslessly without an extra converter. ToolUp.Remoting RPC serialisation is automatic via the client transport and is unaffected by this contract.

### AG Grid Enterprise companion

AG Grid Enterprise initialisation lives in a separate companion package (`src/Feliz.AgGrid.Enterprise/`), not in ToolUp.Platform. This separation exists because:
...
| `StorageScope.fs` | `StorageScope`, `ScopeResolutionRequest`, `ScopeResolutionError` (the historical `PlatformMode` DU was retired in favour of `ServerConfig.Surfaces` + `AccessContext.Subject`) |
...
| `SDK.Shared.fs` | `ServerConfig` — the record only since Phase 347 (incl. `ModuleConfigs`, `RequireHttps`, `TrustForwardedHeaders`, `StaticPathBehaviour`, `SlowRequestThreshold`, `DefaultTeamStorageQuotaBytes`, `RateLimit`); its `defaults` + `fromEnv` live in `Config/ServerConfigFromEnv.fs`, the `*Mode` DUs it selects between (`EventStoreMode`, `StaticPathBehaviour`, `RateLimitConfig`, …) under `Config/`, and `ModuleEvent`, `IEventStore`, `EventRetentionPolicy`, `EventReplay` in `ModuleEvents.fs` |
| `AuditTypes.fs` | `AuditEvent` DU + `AuditEvent.eventTypeName` (the union only since Phase 347); the per-case payload records (`UserLoggedInPayload` etc) live in `Audit/*AuditPayloads.fs` by subsystem lane, `AuditSourceModule` + `AuditSubject` in `Audit/AuditSubject.fs`, and `AuditEnvelope` / `IAuditLog` in `Audit/AuditEnvelope.fs`; reused by middleware, handlers, and `IAuditLog` consumers |
