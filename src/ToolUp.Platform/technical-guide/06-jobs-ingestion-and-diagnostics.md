...
3. Walks every scope returned by `IJobStore.ListScopesWithJobs`, collects the cron-triggered jobs whose `NextRunAt` predates the pause, and emits one `JobSchedulerTickMissed` operational event under the `_platform.jobs` scope with payload `{ ExpectedTickAt; ObservedTickAt; DriftMs; MissedTickCount; JobsSkipped }`. The `_platform.jobs` operational-event stream is deliberately separate from the `AuditEvent` DU — scheduler operational events go through `IEventStore.Write` with `SourceModule = JobsSourceModule`, not through `IAuditLog.Record`, so they do not gain an `AuditEnvelope` wrapper or replicate through audit sinks. A sink that needs both can correlate by `ScopeId + OccurredAt`.
