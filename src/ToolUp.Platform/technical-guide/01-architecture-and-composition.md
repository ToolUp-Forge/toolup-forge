...
| `TrustForwardedHeaders` | `false` | Register `app.UseForwardedHeaders(...)` honouring `X-Forwarded-Proto` / `X-Forwarded-For`. Required behind a TLS-terminating reverse proxy so secure-cookie scoping, OIDC `RedirectUri` generation, and `Url.IsAbsoluteUri` see the originating scheme. | `TOOLUP_TRUST_FORWARDED_HEADERS` |
...
**CSRF token lifecycle.** A 256-bit base64url token is minted per ASP.NET session and surfaced by `GET /api/csrf-token` (`{"Token":"..."}`). The SDK client shell pre-fetches it once at startup (`CsrfClient.prefetch`, called from `SDK.Client.program`) and `UserSession.withRequestHeaders` attaches it as `X-CSRF-Token` to *every* Fable.Remoting call — no per-call-site change. `CsrfMiddleware` fixed-time-compares the header to the session token on state-changing (`POST`/`PUT`/`PATCH`/`DELETE`) `/api/*` requests and returns `403 {"error":"csrf_validation_failed"}` on mismatch. Exemptions: the token endpoint, `PeerRoutePrefixes` (the bearer IS the auth), and `AnonymousRoutePrefixes` (share-token-gated public writes have no session). The cross-origin attacker can ride the session cookie but cannot read the token (CORS protects the GET response) nor guess it.
