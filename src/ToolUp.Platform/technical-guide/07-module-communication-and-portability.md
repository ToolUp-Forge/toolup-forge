...
| `AICancellationDispatchInstanceValidator` | AI is composed + `ReplicaCount > 1` (Phase 6n). `AICancellationRegistry` and `ClientToolDispatchRegistry` are per-process singletons with no cross-instance routing: a `POST /api/ai/cancel/{taskId}` or a client-tool-result POST that lands on a replica other than the one running the agent loop 404s — the Cancel button silently does nothing (the agent keeps spending) and client-tool round-trips hang until the agent-loop timeout. Registered only by the AI compose branch, so non-AI deployments are out of scope at any replica count. The escape hatch **degrades the refusal to a Warning** rather than clearing it: sticky routing is an operator attestation about a load balancer, not something the SDK can verify, so the residual risk (affinity dropped mid-loop) stays visible in the Validators panel. | `AcceptStickyRoutedAiInMultiInstance` — attests that a sticky-session LB pins cancel/result to the owning replica (or run a single AI replica). A distributed registry is roadmap work. | `TOOLUP_ACCEPT_STICKY_ROUTED_AI_MULTI_INSTANCE=1` (+ `TOOLUP_REPLICA_COUNT=N`) |
...
| `AnonymousAIModeValidator` | AI is composed + some `Surfaces` entry is `Anonymous` + no `RateLimit` policy resolves for `AnonymousKind` + the composed `IAIProviderFactory` reports at least one `PlatformDescriptors` entry (**refusal**, Phase 6m) — an unauthenticated caller drives a provider the DEPLOYMENT pays for, with no identity to attribute the spend to, rate-limit per, or cut off. The rate-limit conjunct is `RateLimitConfig.policyFor … AnonymousKind`, not `isEnabled`: a deployment limiting only `UserKind` has a limiter registered and an unlimited anonymous surface. The BYOK-only shape is exempt by construction — with no platform-paid provider wired, an anonymous caller (who has no secret scope to hold a key) can only ever get `NoProviderConfigured`. The escape hatch **degrades the refusal to a Warning** rather than clearing it, for the same reason as `AICancellationDispatchInstanceValidator`: per-IP gating at a proxy is an operator attestation about infrastructure the SDK cannot verify. | `AcceptAnonymousModeWithAI` — attests that per-IP gating or request budgets bound the cost upstream. The two other exits are a rate-limit policy covering `AnonymousKind` (`TOOLUP_RATE_LIMIT_PERMITS=N`) and a BYOK-only `IAIProviderFactory`. | `TOOLUP_ACCEPT_ANONYMOUS_MODE_WITH_AI=1` |
...
| `ForwardedHeadersTrustValidator` | `Mode != Anonymous` + `TrustForwardedHeaders = true` + empty `TrustedProxyCidrs` + no escape hatch (**Error**, Phase 325) — `X-Forwarded-*` from any peer lets a caller rotate `X-Forwarded-For` (bypassing IP rate limiting, poisoning audit logs) and spoof `X-Forwarded-Proto: https` (fooling `Request.IsHttps` → cookie-secure / OIDC-RedirectUri / TLS-branching code). Anonymous-only surfaces keep the Phase 6l.K warning. | Scope the trust: `TOOLUP_TRUSTED_PROXY_CIDRS=10.0.0.0/8` (the terminator's network) — **or** `AcceptForwardedHeadersFromAnyProxy` when a single header-stripping proxy fronts every path | `TOOLUP_ACCEPT_FORWARDED_HEADERS_FROM_ANY_PROXY=1` |
...
**`SkipPreflight` is not a master off-switch.** `ServerConfig.SkipPreflight = true` is the emergency-boot lever for a noisy *companion* probe — it skips the non-security-class validators only. The auth / secret / CSRF / provenance / cross-instance-auth-state guards always run and still abort startup on `Error`. A validator opts into this set by also implementing the `ISecurityClassValidator` marker interface (the always-run set is a type-test over the validators themselves, so a newly-authored security validator can't drift out of it), covering `header-auth-mode`, `oidc-config-completeness`, `oidc-audience-binding`, `sse-auth-mode`, `encrypted-secret-store-mode`, `oauth-secret-encryption-mode`, `oauth-state-store-instance`, `per-scope-key-resolver-distributed`, `auto-bootstrap-dev-admin-mode`, `csrf-default-mode`, `cors-config`, `forwarded-headers-trust`, and `share-token-signing-key-provenance`. The names of every skipped validator are logged at `Warn`, so a `SkipPreflight` boot is visible in the deployment log, not just the `/dev/inspect` panel. A single boolean must never silently disable identity-spoofing / unauthenticated-access protection.
...
The platform signing key (32 bytes) lives in `ISecretStore` under the reserved `_platform` scope as `share_token_signing_key`. It is an **operator-managed secret**: pre-provision it (a base64url-encoded 32+ byte value) before first boot. A production-shaped deployment — `PublicBaseUrl` set or `ReplicaCount > 1` — **refuses to start** when it is absent, unless `TOOLUP_ACCEPT_EPHEMERAL_SHARE_TOKEN_KEY=1` acknowledges a throwaway key. On the acknowledged path (dev / CI / single instance) the store auto-generates and persists one on first use, marks it `share_token_signing_key_origin = auto-generated` so preflight can tell it apart from a provisioned key, and adopts the store's persisted value after writing so concurrent replicas converge on one key. Rotation: overwrite the secret via the secret store's normal Set flow — no restart needed, the key is picked up within the 10-minute cache TTL; all previously-issued tokens then fail verification (no separate "key id" field — single key per deployment in v1). Operator procedure: `DEPLOYMENT.md`, "Share-token signing key".
