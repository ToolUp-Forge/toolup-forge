...
3. **`NotificationHandler.notificationHandler`** (`Server/NotificationHandler.fs`) — the Giraffe HTTP handler at `/api/notifications`. Per connection, it resolves scopeId from `ctx.Items` / query / header (falling back to `"anonymous"`), registers with `SSEConnectionManager`, subscribes a callback on `INotificationChannel`, and runs a 15-second keepalive comment loop on its own response. On disconnect it unsubscribes and removes the connection.
