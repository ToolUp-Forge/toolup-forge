...
- **`_platform.encryption.key-destroyed`** (Phase 22b) — a `CustomNotification` published by `PerScopeKeyResolver` after a successful `DestroyKey`, on the reserved `_platform` topic. Subscribed to by every other replica's resolver, which evicts its cached key material for the named scope so no replica can keep serving a key the crypto-shred destroyed. Crosses the per-scope boundary for the same reason `MembershipChanged` does — and here the boundary crossing is the *point*: a tenant offboarding for GDPR or contract termination is complete on the serving replica when the call returns, and only minute-grain across the fleet, which is exactly what this broadcast closes. Payload: `{ ScopeId; KeyId; RequestedBy; RequestedAt; OriginReplicaId }` — **identifiers and an instant, never key material**, so the envelope is safe to log and safe to carry over a shared bus. `OriginReplicaId` drives echo suppression on the same grounds as the rotation envelope below: the in-process channel delivers a publish back to its publisher, and a replica that acknowledged its own broadcast would record a spurious "another replica saw it" forensic event in a single-replica deployment. Declared as `[<Literal>] KeyDestroyedNotification.NotificationKey`.
- **`_platform.webhooks.secret-rotated`** (Phase 464) — a `CustomNotification` published by `BlobWebhookRegistry` after a successful `RotateSecret`, on the reserved `_platform` topic. Subscribed to by every other instance's registry, which drops its cached webhook signing material for the named scope so the next resolve reads the rotated value. Crosses the per-scope boundary for the same reason `MembershipChanged` does: the invalidation has to reach caches on instances that are not handling the rotating request. Payload: `{ ScopeId; SubscriptionId; CurrentSecretRef; PreviousSecretRef; GraceExpiresAt; RotatedAt; OriginReplicaId }` — **references only, never a secret value**, so the envelope is safe to log and safe to carry over a shared bus. `OriginReplicaId` drives echo suppression: the in-process channel delivers a publish back to its publisher, and a receiving instance discards a broadcast bearing its own identity. Declared as `[<Literal>] WebhookSecretRotatedNotification.NotificationKey`. See ["Signing-secret rotation across instances"](#signing-secret-rotation-across-instances-phase-464) below.
...
### Signing-secret rotation across instances (Phase 464)

Rotation is **eventually consistent across instances, not instant** — and the window is the active `INotificationChannel` companion's fanout latency, which is minute-grain per that interface's precision contract.

**What is actually stale, and why it needed a broadcast.** The dispatcher holds no subscription cache: `dispatchToScope` re-lists the scope per event, and `resolveSigningSecrets` reads `ISecretStore` per delivery. The stale copy is one layer down, in a **caching** `ISecretStore`. `FileSecretStore` memoises a scope's whole secret map on first read and evicts only on its own `SetSecret` / `DeleteSecret`, with **no TTL** — so a secret rotated on instance A would otherwise stay cached on instance B for the life of B's process. There is no periodic refresh to wait out, which is why shortening a cache lifetime was not an available fix.

**What the failure looks like unwired**, and why it does not announce itself: instance A rotates and starts signing with the new secret; instances B…N keep signing with the superseded one. A receiver that rotated in step with the platform verifies A's deliveries and **rejects B…N's as inauthentic** — reported as a bad signature, i.e. as a suspected forgery, not as stale configuration. Deliveries keep succeeding from one instance the whole time, so the pattern reads as intermittent rather than as a rotation problem.

**The wiring rule:**

| Instances | `WireToChannel` | Channel |
|---|---|---|
| 1 | Optional — no sibling cache exists, and the rotating process evicted its own on the `SetSecret` write | In-process default is correct |
| > 1 | **Wire it** | **Distributed companion required** (`ServerConfig.Notifications = RedisNotifications "…"`) — the in-process channel reaches only the publishing process |

`compose` calls `WireToChannel` for you (`ComposeJobs.wireWebhookRegistryToNotificationChannel`), passing the composed `ISecretStore` as the invalidation target. A subscribe failure is best-effort but not silent: it logs `Error` and registers a `webhook-secret-rotation-fanout` degraded-capability entry, so it surfaces on a non-empty `/health` degraded set. An **unwired** rotation is counted on `BlobWebhookRegistry.UnwiredRotateSecretCount` and the first one per process logs a security-class warning naming the consequence and the remedy — counted as well as logged because the one-argument constructor has no logger, and the count needs none to be true. `ObservedRotationCount` counts *sibling* rotations acted on, and echo suppression keeps it at `0` on a single instance, which is what makes a non-zero value evidence that the fanout crossed an instance boundary rather than that a cache was merely cold.

**Invalidation is idempotent and order-insensitive**, so portability rule 5 (no cross-shard ordering) is satisfied: duplicate or out-of-order envelopes converge on the same state — every instance has dropped its memoised copy and will re-read. A distributed companion may fan out per-shard with no cross-shard sequencing promise.

**The seam.** A caching store opts in by implementing `ISecretCacheInvalidation` (`InvalidateScope: string -> unit`) — deliberately a separate interface rather than a fifth `ISecretStore` member, so every existing store keeps compiling (GP 11) and so the type test stays a truthful question. A store that round-trips its vault per call has nothing to invalidate and correctly implements nothing; `WebhookRegistry.secretCacheInvalidator` degrades to `ignore` for it.

**A caller holding a secret by value is outside what this can fix.** The broadcast invalidates *caches*; it cannot reach a value already closed over. A component that resolves a signing secret once at compose must be restarted to pick up a rotation — see the Stripe companion's `WebhookSigner.verifyWithFetcher` for the per-call-resolution shape that avoids the problem entirely.
