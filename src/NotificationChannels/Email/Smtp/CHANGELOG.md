# Changelog — ToolUp.Platform.NotificationChannels.Email.Smtp

All notable changes to the `ToolUp.Platform.NotificationChannels.Email.Smtp` package are recorded here.
...
- Vendor exceptions arriving wrapped in `AggregateException` are now
  ...
  (2026-08-27) proved live in the AWS companions. MailKit's
  connect/authenticate/disconnect calls are non-generic `Task` awaits
  — the highest-risk shape — and a wrapped `SmtpCommandException`
  would have lost the mechanical 4xx-transient / 5xx-permanent
  classification to the generic transient arm.
