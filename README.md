...
Packages publish to [nuget.org](https://www.nuget.org/packages?q=ToolUp) — `dotnet add package ToolUp.Platform.Core` works with no feed wiring and no authentication.
...
- **A proved core.** The dispatch ring buffer every reentrant `dispatch` waits in is proved a FIFO queue through every grow — every deferred message comes out once, in order, and never as a placeholder slot — and the subscription diff run after every `update` is proved to start exactly the new subscriptions, stop exactly the removed ones and keep exactly the common ones, with its `keys = newKeys` shortcut proved to agree with the general path. Both are F\* models extracted to F# and held against the shipped code on **both** hosts — live on .NET, and as the model's recorded verdicts replayed against the Fable transpilation — with the go-red cases committed. Upstream's structures, unchanged; the theorems are the fork's. The capacity floor was reconciled to the proof's precondition (2, stated once as `RingBuffer.MinimumCapacity`) along the way. Ladder and machinery: [`proofs/README.md`](proofs/README.md#the-claims-ladder--the-elmish-runtime-phase-788).
