...
module PageAmbient =
    ...
    ///
    /// `oidcIssuerUrl` rather than `issuerUrl` on purpose: BOTH
    /// `EntraExternalIdConfig` and `EntraExternalIdClientConfig` export
    /// a function called `issuerUrl`, and those modules are opened by
    /// blocks further down the page. A block's own `open` lines are
    /// spliced AFTER this preamble, so such an open shadows anything
    /// declared here — the collision is currently harmless only because
    /// the two consuming blocks happen to sit above it, which is luck,
    /// not a property.
