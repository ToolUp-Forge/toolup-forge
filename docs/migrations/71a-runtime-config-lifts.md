...
- **71.A.11** — hybrid structured-payload case-flips (`EventStore`, `JobScheduler`, `DataSubjectRequests`, `ConversationStore`, `PublicRendering`, `RateLimitStore`, `AdPanel`, `ConsentProvider`): the env-var selects the case, the payload comes from `defaults` / a curated factory; fail-loud when a selected case needs a value not in defaults.
