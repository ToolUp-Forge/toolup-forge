...
## Increment 6 (71.A.9 — client admin-module case-flips) — completes the client tier

Ten client admin-module / profile fields now resolve from Vite defines (Vite > default; no override member): the 8 admin-module mode DUs (`TeamManager`, `TeamConfig`, `PlatformAdmin`, `PermissionsAdmin`, `HealthMonitor`, `ServiceStatusBoard`, `DataSubjectRequestAdmin`, `ToastCentre`) lift their `No*`/`Default*` case-flip, plus the nilary `PlatformAdminProfile` and `InputsPaneWidth`. The hybrid DUs' `Configured*`/`External*`/`Custom*` cases carry function values and **stay compile-time** — an unset / unrecognised / `Configured`-shaped token leaves the config's existing value untouched, so a code-set posture is never clobbered. (`PremiumModel` is single-case — nothing to flip.) Ten new `BundleConstants` accessors + a pure `applyAdminModuleConstants` resolver (built on the generic `parseNoDefault` / `parseCaseToken`) folded onto the built `ClientConfig` inside `fromBundleConstants`. See [the admin-module table](../operations/env-vars.md#client-admin-module--profile-toggles-phase-71a9). Verified by Expecto cases on the token resolvers + a clean `samples/MinimalClient` Fable transpile (the new defines emit for Vite substitution).

## Increment 7 (71.A.11 — hybrid case-flips) — completes Phase 71.A

The hybrid DUs whose enabled case carries a payload. A new `parseHybridCase` helper maps each token to either `Ok value` (a nilary / curated-default case, constructed directly) or `Error why` (a payload-bearing case that **fails loud** at startup naming how to supply the payload — never silently defaulted). Two DUs the audit grouped as HY are actually fully nilary in-tree (`JobScheduler`, `RateLimitStore`) and lift completely. `EventStore` `persistent` uses `EventRetentionPolicy.ninetyDays` as a curated default. `ConversationStore` / `PublicRendering` / `DataSubjectRequests` enable-direction fail loud (retentionDays / ContentRoot path / explicit ErasurePolicy aren't safely defaultable — the last deliberately so, since enabling a compliance subsystem shouldn't be an env-var side-effect). Client `AdPanel` / `ConsentProvider` lift the off-direction only via Vite defines (`applyHybridClientConstants`). See [the hybrid-toggle table](../operations/env-vars.md#hybrid-subsystem-toggles-phase-71a11). Verified by Expecto cases (select + fail-loud + unset→default) + a clean `samples/MinimalClient` Fable transpile.

**With this, Phase 71.A is complete** — every `ServerConfig`/`ClientConfig` field the Phase 71 audit classified RT (and the case-flip of every HY field) now resolves at runtime, server via `TOOLUP_*` env vars and client via `__TOOLUP_*__` Vite defines.

## Deferred follow-up (not a 71.A checklist item)

The audit's *recommended* `PublicBaseUrl`-needs-a-token-issuer preflight `Warn` validator (emit a startup warning when a token-issuing companion is composed AND `PublicBaseUrl = None`) is a preflight-validation concern, not a config-resolution one — it rides forward as a TIDY-UP candidate. The issuers already fail loud at issuance time, so it is an observability nicety, not a correctness gap.
