...
- ~~`IDistributedLock` (Phase 9i) cross-silo single-leader coordination.~~ **Shipped for the webhook half — Phase 16a tail.** The dispatcher's failure-state transition is held under `toolup:webhook-failure-state:{scopeId}:{subscriptionId}`. ~~Still deferred: the scheduler's cron-tick election~~ **Shipped — [Phase 766](766-cron-tick-election.md)** (the contended loser skips; the in-lease re-read checks due-ness), though `compose` still binds the in-process lock into the scheduler. Still deferred: that compose wiring, and the durable queue that would give webhook events a cross-silo owner. `ReplicaCount = 1` remains the only safe shape for `WorkerOnly` / `DispatcherOnly`.
