...
- [`remoting-decoder-algebra.md`](remoting-decoder-algebra.md) — the closed MessagePack value model and the total combinators over it: nine cases with no null and a well-founded size measure, one bounded bytes-to-value pass, decoders that refuse a width rather than casting it, how a consumer writes and registers one, the opt-in/fallback rule, the composition-profile facet and its `Verified` / `Observed` split, the trust-boundary statement in full, and the recorded decision on whether the JSON wire adopts the same algebra.
