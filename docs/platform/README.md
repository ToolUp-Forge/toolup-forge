...
- [`external-contacts.md`](external-contacts.md) — reaching recipients who have no account on the deployment: the `ExternalContact` address book, the per-channel opt-in and its GDPR Article 7(1) evidence, the `RecipientId` widening on the transactional envelopes, why consent is a layer OVER Phase 441's preferences rather than the same store (and why one fails open where the other fails closed), what a `NotificationDeliveryRefused` row means that a silent skip does not, and the duplicate rule that stops one person becoming two sends.
...
- [`agent-hosting.md`](agent-hosting.md) — wiring an external agent stack to a deployment over the Model Context Protocol: an agent principal as a service account plus a default-deny set of tool names, why discovery is authorisation-filtered and why a refusal must be indistinguishable from "no such tool", the operator route from enabling service accounts to granting tools, per-agent budgets and token-partitioned rate limits over the shipped ledger and rate-limit substrates (and why both fail open at request time while a composition that cannot count them is refused at startup), and what the interaction trail records — and deliberately does not.
...
- [`remoting-decoder-algebra.md`](remoting-decoder-algebra.md) — the closed MessagePack value model and the total combinators over it: nine cases with no null and a well-founded size measure, one bounded bytes-to-value pass, decoders that refuse a width rather than casting it, how a consumer writes and registers one, the opt-in/fallback rule, the composition-profile facet and its `Verified` / `Observed` split, the trust-boundary statement in full, and — since Phase 799 — the JSON wire the server decodes its arguments from: the lexical value model, the named-field combinators, the bounded pass and the argument-seam registry.
