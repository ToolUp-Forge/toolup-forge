...
| `TOOLUP_ACCEPT_ANONYMOUS_MODE_WITH_AI` | `AcceptAnonymousModeWithAI` | Lowers a startup preflight refusal to a warning: an AI deployment with an Anonymous surface, no rate-limit policy for anonymous callers and a platform-paid provider, attested cost-bounded upstream. |
